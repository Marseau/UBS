...
async function generateCompleteConversationsCSV() {
  ...
  try {
    // Get all conversation history records
...
async function validateDataCounts() {
  ...
  try {
    ...
    // Get sample conversation to show session_id extraction
    const { data: sampleConv, error: sampleError } = await supabase
      .from('conversation_history')
      .select('conversation_context')
      .not('conversation_context', 'is', null)
      .limit(1);

    if (!sampleError && sampleConv.length > 0) {
      console.log(`📊 Sample session_id: ${sampleConv[0].conversation_context?.session_id}`);
    }
