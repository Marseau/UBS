...
import { StructuredLoggerService } const { } = require("../utils/structured-logger.service");
import { getAdminClient } const { } = require("../config/database");
import winston, { Logger } from 'winston';
import { ValidatedMetricsCalculatorService } from './tenant-metrics/validated-metrics-calculator.service';
...
class TenantMetricsCronOptimizedService {
    ...
    constructor() {
        ...
        // Initialize validated metrics calculator
        this.validatedCalculator = new ValidatedMetricsCalculatorService(this.winstonLogger);
    ...
    async calculateComprehensiveMetrics(): Promise<void> {
        ...
        try {
            ...
            // Process all periods (7d, 30d, 90d) in parallel for maximum efficiency
            const periods = ['7d', '30d', '90d'];
            const periodPromises = periods.map(period => 
                this.processTenantsByPeriod(tenants, period)
            );
            ...
            results.forEach((result, index) => {
                if (result.status === 'fulfilled') {
                    ...
                    totalErrors += tenants.length; // Consider all as errors for this period
    ...
    /**
     * Process tenants for a specific period with intelligent batching
     */
    private async processTenantsByPeriod(
        ...
        try {
            ...
            // Pre-calculate platform totals once per period for efficiency
            const platformTotals = await this.cache.get(`platform_totals_${period}`) || 
                await this.calculatePlatformTotals(period);

            // Process tenants with optimized concurrency
            const results = await this.concurrencyManager.processWithConcurrency(
                tenants,
                async (tenant) => {
                    try {
                        // Calculate both metric systems
                        const [result, validatedMetrics] = await Promise.all([
                            this.metricsCalculator.calculateTenantMetrics(
                                tenant.id,
                                period,
                                platformTotals
                            ),
                            this.validatedCalculator.calculateValidatedMetrics(tenant.id, period as '7d' | '30d' | '90d')
                        ]);
                        
                        // Combine both metric systems
                        const combinedResult = {
                            ...result,
                            metricas_validadas: validatedMetrics
                        };
                        
                        // Store in database (optimized batch insert would be ideal)
                        await this.saveMetricsToDatabase(combinedResult);
    ...
    private async saveMetricsToDatabase(metrics: any): Promise<void> {
        await this.dbPool.withConnection(async (client) => {
            // Separate validated metrics from main metrics
            const { metricas_validadas, ...mainMetrics } = metrics;
            ...
            const { error } = await client
                ...
                .upsert({
                    ...
                    metric_data: mainMetrics,
                    metricas_validadas: metricas_validadas || {},
    ...
    private async aggregateToPlatformMetrics(): Promise<void> {
        ...
        try {
            ...
            // Agregar para todos os períodos em paralelo
...
module.exports = TenantMetricsCronOptimizedService;