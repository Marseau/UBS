-- =====================================================
-- DEFINITIVA TOTAL - ALL CRITICAL ISSUES FIXED
...
DROP FUNCTION IF EXISTS calculate_tenant_metrics_definitiva_total_fixed(date, uuid);

CREATE OR REPLACE FUNCTION calculate_tenant_metrics_definitiva_total_fixed(
...
BEGIN
    RAISE NOTICE 'Starting DEFINITIVA TOTAL FIXED version';
    ...
    LOOP
        ...
        LOOP
            ...
            BEGIN
                RAISE NOTICE 'Processing tenant % for period % (fixed date window: % to %)', 
                    LEFT(v_tenant_record.id::text, 8), v_period_days, v_start_date, v_end_date;
                
                -- =====================================================
                -- CALCULATE PLATFORM TOTALS (FIX 1: Correct date ranges)
                -- =====================================================
                
                -- Platform appointment revenue (FIXED: proper date range + timestamptz handling)
                SELECT COALESCE(SUM(
                    CASE 
                ...
                AND start_time < (v_end_date + 1)::timestamptz  -- FIX: < next day instead of <= end day
                AND status IN ('completed', 'confirmed')
                AND (p_tenant_id IS NULL OR tenant_id = p_tenant_id);
                
                -- Platform appointments (FIXED: proper date range)
                SELECT COUNT(*)
                ...
                -- Platform customers (FIXED: proper date range)
                ...
                -- Platform subscription revenue (FIXED: proper date range)
                ...
                -- FIX 2: Platform conversations (CORRECTED: JSONB access to conversation_history)
                SELECT 
                ...
                -- =====================================================
                -- COLLECT TENANT DATA (FIX 1: Proper date ranges)
                -- =====================================================
                
                -- Appointments breakdown (FIXED: proper date ranges + spelling consistency)
                SELECT 
                    COALESCE(SUM(
                        CASE 
                            WHEN COALESCE(quoted_price, 0) > 0 THEN quoted_price
                ...
                -- Tenant subscription cost (FIXED: proper date range)
                SELECT COALESCE(SUM(amount), 0)
                ...
                -- FIX 2: AI interactions and conversations (CORRECTED: JSONB access)
                SELECT 
                    COUNT(CASE WHEN is_from_user = false THEN 1 END),
                    COUNT(DISTINCT conversation_context->>'session_id'),
                ...
                -- Most popular service (FIXED: proper date range)
                SELECT s.name
                ...
                -- =====================================================
                -- CALCULATE DERIVED METRICS (FIX 5: Proper order)
                -- =====================================================
                ...
                -- Conversation metrics (FIXED: use corrected conversation duration)
                ...
                -- FIX 5: Calculate participation metrics AFTER platform totals are calculated
                ...
                -- FIX 5: Health score calculation AFTER participation metrics are calculated
                ...
                v_comprehensive_metrics := jsonb_build_object(
                    ...
                    'conversation_outcomes', jsonb_build_object(
                        ...
                        'messages_per_conversation', CASE WHEN v_tenant_conversations > 0 
                            THEN (SELECT COUNT(*) FROM conversation_history 
                                  WHERE tenant_id = v_tenant_record.id
                                  AND created_at >= v_start_date::timestamptz
                    ...
                    'metadata', jsonb_build_object(
                        ...
                        'data_source', 'definitiva_total_fixed_v4.0',
                        'fixes_applied', ARRAY['date_window_corrected', 'conversation_jsonb_fixed', 'variable_order_fixed', 'spelling_consistent'],
                ...
                RAISE NOTICE 'DEFINITIVA TOTAL FIXED: Processed tenant % for % period - Revenue=%, Health=%, Conversations=%', 
    ...
    v_result := json_build_object(
        ...
        'modules_included', 8,
        'calculation_date', p_calculation_date,
        'version', 'DEFINITIVA_TOTAL_FIXED_v4.0',
        'fixes_applied', ARRAY['date_window_corrected', 'conversation_jsonb_fixed', 'variable_order_fixed', 'spelling_consistent'],
        'execution_time_ms', EXTRACT(MILLISECONDS FROM clock_timestamp() - v_execution_start)
    );
    
    RAISE NOTICE 'DEFINITIVA TOTAL FIXED calculation completed: % tenants × 3 periods = % records with % metrics each in %ms', 
        v_processed_count, 
        v_processed_count * 3,
        73,
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'ERROR in DEFINITIVA TOTAL FIXED metrics calculation: % - %', SQLSTATE, SQLERRM;
...
GRANT EXECUTE ON FUNCTION calculate_tenant_metrics_definitiva_total_fixed(date, uuid) TO authenticated;