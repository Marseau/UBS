-- =====================================================
-- OPTIMIZED PLATFORM METRICS FUNCTION
-- =====================================================
-- Performance-optimized version of calculate_enhanced_platform_metrics
...
BEGIN
    ...
    RAISE NOTICE '🚀 Starting OPTIMIZED platform metrics calculation for period: % to %', v_start_date, v_end_date;
    
    -- =====================================================
    -- OPTIMIZATION 1: SINGLE AGGREGATED QUERY APPROACH
    -- =====================================================
    -- Instead of looping through each tenant, use aggregated queries
    
    RAISE NOTICE '⚡ Using optimized aggregation queries...';
    ...
    WITH tenant_metrics AS (
        SELECT 
            ...
            -- Conversation metrics (optimized with indexes)
            ...
            COALESCE(conv_stats.chat_minutes, 0) as chat_minutes,
        ...
        -- LEFT JOIN optimized conversation statistics
        LEFT JOIN (
            SELECT 
                tenant_id,
                ...
                -- Simplified chat duration calculation
                COALESCE(SUM(EXTRACT(EPOCH FROM (
                    LEAD(created_at) OVER (PARTITION BY tenant_id, user_id ORDER BY created_at) - created_at
                )) / 60) FILTER (WHERE message_type = 'user'), 0)::INTEGER as chat_minutes
    ...
    -- Calculate advanced metrics in CTE
    enhanced_metrics AS (
        SELECT 
            *,
            -- Revenue/Usage ratio
            CASE WHEN chat_minutes > 0 
                THEN (monthly_revenue / chat_minutes) 
            ...
            CASE WHEN conversations > 0 
                THEN (
                    (LEAST(appointments * 100.0 / conversations, 100) * 0.4) +
                    (LEAST(valid_conversations * 100.0 / conversations * 100, 100) * 0.3) +
                    (CASE WHEN monthly_revenue / GREATEST(chat_minutes, 1) BETWEEN 0.5 AND 2.0 THEN 100 ELSE 50 END * 0.3)
                )
    ...
    RAISE NOTICE '✅ OPTIMIZED calculation completed! Tenants: %, Time: %ms', v_processed_tenants, v_execution_time;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE NOTICE '❌ Error in optimized calculation: % - %', SQLSTATE, SQLERRM;
...
-- =====================================================
-- PERFORMANCE ANALYSIS HELPER FUNCTION
-- =====================================================

CREATE OR REPLACE FUNCTION analyze_function_performance()
RETURNS TABLE (
    function_name TEXT,
    execution_time_ms INTEGER,
...
BEGIN
    ...
    RETURN QUERY SELECT 
        'original'::TEXT, v_original_time, 0::DECIMAL(5,2)
    UNION ALL
    SELECT 
        'optimized'::TEXT, 
        v_optimized_time, 
        ((v_original_time - v_optimized_time) * 100.0 / v_original_time)::DECIMAL(5,2);
END;
$$;
...
'Optimized version of platform metrics calculation with:
...
COMMENT ON FUNCTION analyze_function_performance IS 
'Helper function to compare performance between original and optimized
platform metrics calculation functions. Returns execution times and improvement percentage.';