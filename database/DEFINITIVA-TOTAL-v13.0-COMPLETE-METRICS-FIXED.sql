-- =====================================================
-- DEFINITIVA TOTAL v13.0 - COMPLETE 73+ METRICS WITH v12.0 FIXES
-- Combines: Full original 73+ metrics + v12.0 working fixes
-- =====================================================

DROP FUNCTION IF EXISTS calculate_tenant_metrics_definitiva_total_v13(date, uuid);

CREATE OR REPLACE FUNCTION calculate_tenant_metrics_definitiva_total_v13(
...
BEGIN
    RAISE NOTICE 'Starting DEFINITIVA TOTAL v13.0 - COMPLETE 73+ METRICS WITH v12.0 FIXES';
    ...
    LOOP
        ...
        LOOP
            ...
            BEGIN
                -- Calculate period dates (ORIGINAL METHOD)
                ...
                -- Platform customers (v12.0 FIX: use user_id not customer_id)
                ...
                SELECT COUNT(DISTINCT user_id) -- v12.0 FIX: user_id not customer_id
                ...
                -- =====================================================
                -- BUILD COMPREHENSIVE JSONB METRICS (ALL 73+ METRICS)
                -- =====================================================
                ...
                v_comprehensive_metrics := jsonb_build_object(
                    ...
                    'metadata', jsonb_build_object(
                        ...
                        'data_source', 'definitiva_total_v13',
                ...
                -- =====================================================
                -- STORE COMPREHENSIVE METRICS - v12.0 WORKING FIX!
                -- =====================================================
                
                -- v12.0 FIX: Correct store_tenant_metric call with proper parameters
                ...
                RAISE NOTICE 'DEFINITIVA TOTAL v13.0: Processed tenant % for %d period - Revenue=%, Health=%, Conversations=%, Total Metrics=73', 
    ...
    v_result := json_build_object(
        ...
        'modules_included', 8,
        'calculation_date', p_calculation_date,
        'version', 'DEFINITIVA_TOTAL_v13.0_COMPLETE_METRICS_FIXED',
        'fix_applied', 'v12.0_working_fixes_with_complete_73_metrics',
        'execution_time_ms', EXTRACT(MILLISECONDS FROM clock_timestamp() - v_execution_start)
    );
    
    RAISE NOTICE 'DEFINITIVA TOTAL v13.0 calculation completed: % tenants × 3 periods = % records with 73 metrics each in %ms', 
        v_processed_count, 
        v_processed_count * 3,
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'ERROR in DEFINITIVA TOTAL v13.0: % - %', SQLSTATE, SQLERRM;
...
GRANT EXECUTE ON FUNCTION calculate_tenant_metrics_definitiva_total_v13(date, uuid) TO authenticated;