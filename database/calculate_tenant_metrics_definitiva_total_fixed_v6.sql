-- =====================================================
-- DEFINITIVA TOTAL v6.0 - CORREÇÃO DO TIMING BUG
-- Fix 1-8: Mantidos da v5.0 (todos os fixes anteriores preservados)
-- Fix 9: TIMING BUG CORRIGIDO - Verificação robusta com count before/after + retry
-- =====================================================
...
DECLARE
    ...
    -- FIX 9: Variáveis para correção do timing bug
...
BEGIN
        ...
        LOOP
            ...
            LOOP
                BEGIN
                    DECLARE
                        ...
                        -- All metrics variables (same as v5.0)
                    ...
                    BEGIN
                        ...
                        -- =====================================================
                        -- DERIVED METRICS CALCULATION (Same as v5.0 but protected)
                        -- =====================================================
                        ...
                        v_comprehensive_metrics := jsonb_build_object(
                            ...
                            'metadata', jsonb_build_object(
                                ...
                                'data_source', 'definitiva_total_fixed_v6.0',
                                'retry_attempt', v_retry_count + 1,
                                'fixes_applied', ARRAY['date_window_corrected', 'conversation_jsonb_fixed', 'variable_order_fixed', 'spelling_consistent', 'exception_handling_improved', 'protected_calculations', 'retry_mechanism', 'timing_bug_fixed'],
                        ...
                        -- =====================================================
                        -- FIX 9: TIMING BUG CORRIGIDO - Verificação robusta
                        -- =====================================================
                        
                        -- Contar registros ANTES da tentativa de store
                        ...
                        RAISE NOTICE '[%] BEFORE store: % existing records for period %d', 
                            LEFT(v_tenant_record.id::text, 8), v_record_count_before, v_period_days;
                        ...
                        BEGIN
                            ...
                            RAISE NOTICE '[%] store_tenant_metric call completed for period %d', 
                                LEFT(v_tenant_record.id::text, 8), v_period_days;
                            
                        EXCEPTION WHEN OTHERS THEN
                            RAISE WARNING '[%] store_tenant_metric call failed for period %d: %', 
                                LEFT(v_tenant_record.id::text, 8), v_period_days, SQLERRM;
                            v_store_result := false;
                            CONTINUE;
                        END;
                        
                        -- Verificação robusta DEPOIS (com retry)
                        ...
                        WHILE v_verification_attempts < 5 AND v_store_result = false LOOP
                            ...
                            RAISE NOTICE '[%] VERIFICATION attempt %: BEFORE=%, AFTER=%', 
                                LEFT(v_tenant_record.id::text, 8), v_verification_attempts + 1, 
                                v_record_count_before, v_record_count_after;
                            
                            IF v_record_count_after > v_record_count_before THEN
                                v_store_result := true;
                                RAISE NOTICE '[%] ✅ TIMING BUG FIX SUCCESS: Record verified after % attempts for period %d', 
                                    LEFT(v_tenant_record.id::text, 8), v_verification_attempts + 1, v_period_days;
                                EXIT;
                            END IF;
                        ...
                        IF v_store_result = false THEN
                            RAISE WARNING '[%] ❌ TIMING BUG FIX FAILED: Record not verified after % attempts for period %d (BEFORE=%, AFTER=%)', 
                                LEFT(v_tenant_record.id::text, 8), v_verification_attempts, 
                                v_period_days, v_record_count_before, v_record_count_after;
                        END IF;
            ...
            IF v_store_result = false THEN
                ...
                v_failed_records := v_failed_records || jsonb_build_object(
                    ...
                    'timing_bug_fix_status', 'failed_verification'
                );
    ...
    v_result := json_build_object(
        ...
        'modules_included', 8,
        'calculation_date', p_calculation_date,
        'version', 'DEFINITIVA_TOTAL_FIXED_v6.0_TIMING_BUG_FIXED',
        'improvements', ARRAY['retry_mechanism', 'protected_calculations', 'detailed_logging', 'storage_verification', 'error_tracking', 'timing_bug_count_verification'],
...
-- SELECT calculate_tenant_metrics_definitiva_total_fixed_v6();