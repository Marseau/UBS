    ...
    if (currentStep === 'ask_additional_data') {
      ...
      if (/\b(não|nao)\b/.test(txt) || ['n', 'nao', 'não'].includes(txt)) {
        ...
        return {
          ...
          telemetryData: {
            intent: 'returning_user_declined_additional',
            confidence: 1.0,
      ...
      if (/\b(sim|s)\b/.test(txt)) {
        ...
        return {
          ...
          updatedContext: ctx,
          telemetryData: {
            intent: 'returning_user_consent_granted',
            confidence: 1.0,
      ...
      return {
        ...
        updatedContext: context,
        telemetryData: {
          intent: 'returning_user_consent_clarify',
          confidence: 1.0,
...
3) Não explique. Não inclua texto extra.`;
  ...
  private async determineUserStatus(userPhone: string, tenantId: string): Promise<{
    ...
    try {
      ...
      // 1. Buscar usuário globalmente (sem filtro de tenant)
