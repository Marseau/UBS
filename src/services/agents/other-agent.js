...
class OtherAgent {
    constructor() {
        ...
        this.agent = {
            id: 'other_agent',
            name: 'Agente para Outros Serviços',
            domain: 'other',
...
Sempre priorize a satisfação do cliente e a eficiência do atendimento!`;
    ...
    async gatherClientInformation(args, context) {
        try {
            const clientProfile = {
    ...
    async checkServiceAvailability(args, context) {
        try {
            ...
            const flexibilitySuggestions = this.generateFlexibilitySuggestions(args.flexibility, filteredSlots, args.service_type);
            const serviceInfo = this.getServiceInfo(args.service_type, context.tenantConfig);
            return {
                success: true,
                data: {
                    available_slots: filteredSlots.slice(0, 20),
    ...
    async bookGeneralAppointment(args, context) {
        try {
            const { data: services, error: servicesError } = await database_1.supabase
                .from('services')
            ...
            if (!service) {
                const { data: newService, error: createError } = await database_1.supabase
                    .from('services')
                    .insert({
                    tenant_id: context.tenantId,
    ...
    async provideBusinessInformation(args, context) {
        try {
            ...
            switch (args.information_type) {
                case 'services':
                    ...
                    informationContent = `🎯 **Nossos Serviços:**\n\n${servicesInfo.map(service => `• **${service.name}** - R$ ${service.price}\n  ${service.description}\n  ⏱️ Duração: ${service.duration} minutos`).join('\n\n')}`;
                    break;
                case 'pricing':
                    informationContent = `💰 **Tabela de Preços:**\n\n${tenant?.services?.map(service => `• ${service.name}: R$ ${service.price}`).join('\n') || 'Entre em contato para orçamento personalizado'}\n\n💳 **Formas de Pagamento:**\n• Dinheiro (5% desconto)\n• PIX (3% desconto)\n• Cartão de crédito/débito\n• Transferência bancária`;
                    break;
                case 'location':
    ...
    async calculateServicePricing(args, context) {
        try {
            const basePricing = {
                'simples': 80,
                'medio': 120,
            ...
            if (args.additional_services) {
                additionalCost = args.additional_services.reduce((sum, service) => sum + (additionalPricing[service] || 20), 0);
            }
    ...
    async handleCustomerInquiry(args, context) {
        try {
            let response = '';
            let actions = [];
    ...
    generateRecommendations(serviceInterest, budgetRange) {
    ...
    generateTimeSlots(date, businessHours) {
    ...
    generateFlexibilitySuggestions(flexibility, slots = [], serviceType = '') {
        const suggestions = [];
    ...
    getServiceInfo(serviceType, tenantConfig) {
        return {
