...
Sempre priorize a ética profissional e o bem-estar do cliente!`;
    ...
    async assessCaseUrgency(args, context) {
        try {
            ...
            let urgencyReasons = [];
            const hasEmergencyKeywords = emergencyKeywords.some(keyword => description.includes(keyword));
    ...
    async checkLegalAvailability(args, context) {
        try {
            ...
            return {
                success: true,
                data: {
                    available_slots: availableSlots.slice(0, 12),
                    consultation_type: args.consultation_type,
                    duration: duration,
                    legal_area: args.legal_area || 'geral'
                },
    ...
    async bookLegalConsultation(args, context) {
        try {
    ...
    async getRequiredDocuments(args, context) {
        ...
        if (areaData.specific && args.case_type && areaData.specific[args.case_type]) {
            documentsMessage += `\n\n**📋 Documentos Específicos para ${args.case_type}:**\n${(0, validation_helpers_1.validateArray)(areaData.specific[args.case_type]).map(doc => `• ${doc}`).join('\n')}`;
        }
    ...
    async calculateConsultationCost(args, context) {
        try {
            const basePrices = {
                'inicial': 300,
                'urgencia': 450,
