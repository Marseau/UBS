...
exports.ConsultingAgent = void 0;
...
Sempre priorize a geração de valor tangível e sustentável para o cliente!`;
    ...
    async assessBusinessNeeds(args, context) {
        try {
            ...
            const riskFactors = args.main_challenges.filter(challenge => ['crise financeira', 'perda de mercado', 'problemas legais', 'falta de caixa'].some(risk => challenge.toLowerCase().includes(risk)));
            const riskLevel = riskFactors.length > 0 ? 'alto' :
                args.urgency_level === 'critica' ? 'médio-alto' : 'baixo';
            const consultingApproach = this.defineConsultingApproach(args.business_stage, args.main_challenges, args.urgency_level);
    ...
    async checkConsultingAvailability(args, context) {
        try {
            ...
            if (args.urgency_level === 'critica') {
                availableSlots.push({ date: today.toISOString().split('T')[0], time: '14:00', type: 'emergencial' }, { date: today.toISOString().split('T')[0], time: '18:00', type: 'emergencial' });
            }
            for (let i = 1; i <= 14; i++) {
            ...
            return {
                success: true,
                data: {
                    available_slots: availableSlots.slice(0, 25),
    ...
    async bookConsultingSession(args, context) {
        try {
            ...
            const appointmentData = {
                ...
                end_time: new Date(new Date(`${args.date}T${args.time}:00`).getTime() + ((0, validation_helpers_1.validateServiceDuration)(service.duration_minutes) * 60000)).toISOString(),
    ...
    async generateBusinessAnalysis(args, context) {
        try {
            ...
            const healthScore = this.calculateBusinessHealthScore(args.financial_health, args.growth_challenges.length, args.company_data);
            const recommendations = this.generateStrategicRecommendations(analysisFramework, args.growth_challenges, healthScore);
    ...
    async recommendConsultingStrategy(args, context) {
        try {
            const phases = this.createPhasedApproach(args.priority_areas, args.timeline_constraints);
            const budgetRecommendations = this.createBudgetRecommendations(args.available_budget, args.priority_areas);
    ...
    async calculateConsultingInvestment(args, context) {
        try {
            const hourlyRates = {
                'estrategia': 400,
                'financeiro': 350,
            ...
            const avgHourlyRate = args.consulting_scope.reduce((sum, area) => sum + (hourlyRates[area] || 350), 0) / args.consulting_scope.length;
    ...
    defineConsultingApproach(stage, challenges, urgency) {
        if (urgency === 'critica')
            return 'Intervenção intensiva com foco em estabilização';
        if (stage === 'startup')
            return 'Consultoria de estruturação e validação';
        if (stage === 'crescimento')
            return 'Consultoria de escalabilidade e otimização';
        if (stage === 'maturidade')
            return 'Consultoria de inovação e competitividade';
        return 'Consultoria estratégica personalizada';
    }
    getComplexityMultiplier(size, sector) {
        const sizeMultipliers = {
            'mei': 1.0, 'micro': 1.1, 'pequena': 1.2, 'media': 1.4, 'grande': 1.8
        };
        const sectorComplexity = {
            'financeiro': 1.3, 'saude': 1.2, 'industrial': 1.1, 'tecnologia': 1.2
        };
        return (sizeMultipliers[size] || 1.2) * (sectorComplexity[sector] || 1.0);
    }
    determineConsultantLevel(specialization, sector) {
    ...
    identifyStrengths(companyData, marketContext) {
        return [
    ...
    identifyWeaknesses(challenges, financialHealth) {
    ...
    identifyOpportunities(marketContext, companyData) {
        return [
    ...
    identifyThreats(marketContext, financialHealth) {
    ...
    createPriorityMatrix(challenges) {
    ...
    calculateBusinessHealthScore(financial, challengeCount = 0, companyData) {
        let score = 7;
        if (financial === 'excelente')
            score += 2;
        else if (financial === 'boa')
            score += 1;
        else if (financial === 'preocupante')
            score -= 1;
        else if (financial === 'critica')
            score -= 3;
        score -= Math.min(challengeCount * 0.5, 3);
        return Math.max(1, Math.min(10, Math.round(score)));
    }
    generateStrategicRecommendations(swot, challenges, healthScore) {
    ...
    identifyQuickWins(challenges, companyData) {
        return [
    ...
    createPhasedApproach(priorities, timeline) {
        return [
            {
                phase: 1,
    ...
    createBudgetRecommendations(budget, priorities = []) {
    ...
    defineRiskMitigation(assessment) {
        return [
...
exports.ConsultingAgent = ConsultingAgent;
//# sourceMappingURL=consulting-agent.js.map