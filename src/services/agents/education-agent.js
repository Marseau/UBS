...
Sempre priorize o desenvolvimento integral do aluno e sua autoestima acadêmica!`;
    ...
    async assessStudentLevel(args, context) {
        try {
            let educationalLevel = 'fundamental1';
            if (args.student_age <= 6)
                educationalLevel = 'infantil';
            else if (args.student_age <= 10)
                educationalLevel = 'fundamental1';
            else if (args.student_age <= 14)
                educationalLevel = 'fundamental2';
            else if (args.student_age <= 17)
                educationalLevel = 'medio';
            else
    ...
    async checkTutoringAvailability(args, context) {
        try {
            ...
            for (let i = 1; i <= 7; i++) {
                ...
                if (date.getDay() === 0 && args.preferred_schedule !== 'fins_semana')
                    continue;
            ...
            return {
                success: true,
                data: {
                    available_slots: availableSlots.slice(0, 15),
                    lesson_type: args.lesson_type,
                    duration: duration,
                    subject: args.subject,
    ...
    async bookTutoringSession(args, context) {
        try {
    ...
    async createStudyPlan(args, context) {
        try {
            const planDurations = {
                '1_mes': 4,
                '3_meses': 12,
    ...
    async recommendStudyResources(args, context) {
        ...
        try {
            ...
            if (args.resource_type && args.resource_type !== 'todos') {
                if (levelResources[args.resource_type]) {
                    recommendedResources[args.resource_type] = (0, validation_helpers_1.validateArray)(levelResources[args.resource_type]);
                }
            }
            else {
    ...
    async trackStudentProgress(args, context) {
        try {
            const progressData = {
    ...
    getMilestoneFocus(subject, level, milestone) {
        const focusMap = {
            'matematica': {
                'fundamental1': [
    ...
    getStudyMaterials(subject, level) {
        const materials = {
    ...
    calculateOverallProgress(scores) {
    ...
    generateRecommendations(improvementAreas) {
        ...
        const specific = improvementAreas.map(area => recommendations[area.toLowerCase()] || `Focar em ${area}`);
        return specific.length > 0 ? specific : defaultRecommendations;
    }
}
exports.EducationAgent = EducationAgent;
//# sourceMappingURL=education-agent.js.map