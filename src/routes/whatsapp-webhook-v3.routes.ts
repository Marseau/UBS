...
class ValidationService {
    ...
    setImmediate(async () => {
      ...
      try {
      // ✅ Tenant ID já vem correto do orchestrator
      ...
      // 🔍 DEBUG: Logs para diagnosticar persistência
      logger.info('Persist debug', { 
        tenantId, 
        phoneNumberId, 
        userPhone,
        intent: result.telemetryData?.intent,
        demoMode: (req as any)?.demoMode,
        contextTenantId: result.updatedContext?.tenant_id 
      });
      
      if (tenantId) {
        // Tentar identificar o usuário por telefone
        let userId: string | null = null;
        try {
          const u = await DatabaseService.findUserByPhone(tenantId, userPhone);
          if (u?.id) userId = u.id;
        } catch {}
        // Se ainda não encontrou, cria/vincula um usuário mínimo por telefone e relação com o tenant
        if (!userId) {
          try {
            const ensuredId = await DatabaseService.upsertUserForTenant(tenantId, userPhone, {} as any);
            if (ensuredId) {
              userId = ensuredId;
            }
          } catch (e) {
            logger.error('Failed to ensure minimal user for demo', { tenantId, userPhone, error: e });
          }
        }
        // Garantir relação user_tenants
        if (userId) {
          try {
            const rel = await supabaseAdmin
              .from('user_tenants')
              ...
              .maybeSingle();
            if (!rel.data) {
              const insRel = await supabaseAdmin.from('user_tenants')
                .insert([{ tenant_id: tenantId, user_id: userId }]);
              if (insRel.error) logger.error('Failed to insert user_tenants relation', { tenantId, userId, error: insRel.error });
            }
          } catch (e) {
            logger.error('Failed to ensure user_tenants relation', { tenantId, userId, error: e });
          }
        }

        // 🚨 CORREÇÃO CRÍTICA: OUTCOME SEMPRE NULL - SISTEMA LEGACY DESABILITADO
        const intent: string | undefined = result.telemetryData?.intent;
        const outcome: string | null = null; // ✅ FORÇAR NULL - OUTCOMES SÃO DETERMINADOS PELO ConversationOutcomeAnalyzerService
        
        console.log('🔧 OUTCOME REAL:', { intent, originalOutcome: result.conversationOutcome, finalOutcome: outcome });
        // Telemetria do orchestrator disponível
        // 🚨 CORREÇÃO CRÍTICA: Usar métricas LLM do WebhookFlowOrchestrator
        const llmMetrics = result.llmMetrics as any || {};
        console.log('🔍 MÉTRICAS LLM CAPTURADAS:', { hasLLMMetrics: !!result.llmMetrics, llmMetrics });
        const totalTokens = llmMetrics.total_tokens ?? null;
        const apiCostUsd = llmMetrics.api_cost_usd ?? null;
        const processingCostUsd = llmMetrics.processing_cost_usd ?? null; // 🚨 ADIÇÃO: Custo de processamento
        const latencyOpenAiMs = llmMetrics.latency_ms ?? null;

        // session_id estável por whatsapp_session_key (conversation_id)
        ...
        if (!sessionDataForContext.conversationId) {
          ...
          sessionDataForContext.conversationStartedAt = new Date().toISOString();
          await cache.setSession(sessionKey, sessionDataForContext);
        }
        const sessionUuid = sessionDataForContext.conversationId as string;
        const startedAtMs = Date.parse(sessionDataForContext.conversationStartedAt || new Date().toISOString());
        const durationMinutes = Math.max(0, Math.floor((Date.now() - startedAtMs) / 60000));
        const base: any = {
          tenant_id: tenantId,
          user_id: userId,
          ...
          // 🚨 CORREÇÃO CRÍTICA: Incluir métricas LLM capturadas do OpenAI
          tokens_used: totalTokens,
          api_cost_usd: apiCostUsd,
          processing_cost_usd: processingCostUsd, // 🚨 ADIÇÃO: Custo de processamento
          confidence_score: result.telemetryData?.confidence || null
        };

        // GARANTIR que conversation_outcome siga a lógica correta
        console.log('🔧 OUTCOME FINAL:', { outcome, willBeNull: outcome === null });
        
        const { error: chError } = await supabaseAdmin.from('conversation_history').insert([
          {
            ...base,
            content: text,
            is_from_user: true,
            ...
            conversation_outcome: null, // 🚨 CORREÇÃO: Mensagem usuário sempre NULL
          ...
          },
          {
            ...
            message_type: 'text',
            intent_detected: intent || null,
            conversation_outcome: outcome, // 🚨 CORREÇÃO: Só preenche se for fim de conversa
            tokens_used: totalTokens,
            api_cost_usd: apiCostUsd,
            processing_cost_usd: processingCostUsd, // 🚨 ADIÇÃO: Custo de processamento
            conversation_context: {
        ...
        if (chError) {
          console.error('🚨 ERRO NO INSERT:', chError);
          logger.error('Failed to persist conversation_history', { error: chError, tenantId, userId, message_source: base.message_source });
        } else {
